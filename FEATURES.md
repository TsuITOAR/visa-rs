...
The async API itself (`Instrument::into_async`, `AsyncInstrument`) is always
available; `tokio` only adds the adapter that plugs it into Tokio's IO traits.

> **`custom-repr` fails to compile unless you configure it.** That is deliberate
> — see [Custom-Repr Feature](#custom-repr-feature). Do not enable it, and do not
> build with `--all-features`, unless you have supplied a mapping.

The rest of this document covers type representation, which is what
`cross-compile` and `custom-repr` control.
...
This uses the `size_of`-based approach, where type sizes are evaluated on the host at
proc-macro compile time. Since every type in the table below is the same width on
every platform NI supports, the host's answer is also the target's, so this works for
cross-compilation too. The explicit features exist to make the mapping auditable and
overridable rather than because the default is wrong.

**Limitations:**

- ✅ Simple and straightforward, no configuration needed
- ⚠️ Trusts that the host and target agree on these widths, which they do today but is not enforced
...
export VISA_REPR_VISTATUS='target_os = "windows":i32,target_os = "linux":i32'
...
# .cargo/config.toml
[env]
VISA_REPR_VISTATUS = 'target_os = "windows":i32,not(target_os = "windows"):i32'
VISA_REPR_VIUINT32 = 'target_os = "windows":u32,not(target_os = "windows"):u32'
VISA_REPR_VIINT32 = 'target_os = "windows":i32,not(target_os = "windows"):i32'
VISA_REPR_VIUINT16 = "u16"
VISA_REPR_VIINT16 = "i16"
VISA_REPR_VIEVENT = 'target_os = "windows":u32,not(target_os = "windows"):u32'
VISA_REPR_VIEVENTTYPE = 'target_os = "windows":u32,not(target_os = "windows"):u32'
VISA_REPR_VIEVENTFILTER = 'target_os = "windows":u32,not(target_os = "windows"):u32'
VISA_REPR_VIATTR = 'target_os = "windows":u32,not(target_os = "windows"):u32'
```
